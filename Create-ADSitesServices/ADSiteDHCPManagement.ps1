#region AD Sites Management Cmdlets
#=============================================================================#  
#                                                                             # 
# Powershell Module for managing AD sites, links, and subnets                 # 
...
function New-ADSite ($SiteName, $Description)
{
    try
    {   # Create the site
        $forest  = [System.DirectoryServices.ActiveDirectory.Forest]::GetCurrentForest()
        $type    = [System.DirectoryServices.ActiveDirectory.DirectoryContextType]"forest"
        $context = New-Object System.DirectoryServices.ActiveDirectory.DirectoryContext($type, $forest) 
        $siteObj = New-Object System.DirectoryServices.ActiveDirectory.ActiveDirectorySite($context, $SiteName) 
        $siteObj.Options=[System.DirectoryServices.ActiveDirectory.ActiveDirectorySiteOptions]::GroupMembershipCachingEnabled 
        $siteObj.Save() | Out-Null
        $sitede  = $siteObj.GetDirectoryEntry() 
        $sitede.Description = $Description
        $sitede.CommitChanges() | Out-Null 
    }
    catch { throw $_ }
}

function New-ADSiteLink ($DefaultSite, $SiteName, $Cost, $ReplicationInterval)
{
    try
    {   # Create site links
        $forest    = [System.DirectoryServices.ActiveDirectory.Forest]::GetCurrentForest()    
        $type      = [System.DirectoryServices.ActiveDirectory.DirectoryContextType]"forest"        
        $context   = new-object System.DirectoryServices.ActiveDirectory.DirectoryContext($type, $forest)
        $link      = new-object System.DirectoryServices.ActiveDirectory.ActiveDirectorySiteLink($context, $SiteName) 
        $siteLink1 = [System.DirectoryServices.ActiveDirectory.ActiveDirectorySite]::FindByName($context, $SiteName)  
        $siteLink2 = [System.DirectoryServices.ActiveDirectory.ActiveDirectorySite]::FindByName($context, $DefaultSite)  # Currently the default site is hard-coded. I will update this to use parameter input.
        $link.Sites.Add($siteLink1) | Out-Null 
        $link.Sites.Add($siteLink2) | Out-Null  
        $link.Cost = $Cost         
        $link.ReplicationInterval = $ReplicationInterval       
        $link.Save() | Out-Null
    }
    catch { throw $_ }
}

function New-ADSubnet ($SiteName, $Subnet, $Description, $Location)
{
    try
    {
        $forest      = [System.DirectoryServices.ActiveDirectory.Forest]::GetCurrentForest()
        $type        = [System.DirectoryServices.ActiveDirectory.DirectoryContextType]"forest"  
        $SubnetDN    = ("CN=Subnets,CN=Sites," + ([ADSI]"LDAP://RootDSE").configurationNamingContext)          
        $NewSubnetDN = ("CN=" + $Subnet +"," + $SubnetDN)
        $context     = new-object System.DirectoryServices.ActiveDirectory.DirectoryContext($type, $forest)           
        $subnet      = new-object System.DirectoryServices.ActiveDirectory.ActiveDirectorySubnet($context, $Subnet, $SiteName) 
        $Subnet.Location = $Location 
        $subnet.Site = [System.DirectoryServices.ActiveDirectory.ActiveDirectorySite]::FindByName($context,$SiteName)
        $subnet.Save() | Out-Null             
        #Set the Subnet Description 
        $subnetde    = $subnet.GetDirectoryEntry() 
        $subnetde.Description = $Description 
        $subnetde.CommitChanges() | Out-Null    
    }
    catch { throw $_ }  
}

function WriteLog ($Value, $Path, [Parameter(ParameterSetName=1)][switch]$Start, [Parameter(ParameterSetName=2)][switch]$End)
{
    if(![System.IO.Directory]::Exists($Path)){[System.IO.Directory]::CreateDirectory($Path)}

    if ($Start)
    {Add-Content -Value "[Start $((Get-Date).ToString())] [Initiated by $env:USERNAME] [$Value]" -Path "$Path\Create-Site_error.log" -Force }
    elseif ($End)
    {Add-Content -Value "[End $((Get-Date).ToString())]" -Path $Path}
    else
    { Add-Content -Value $Value -Path $Path -Force}
}

function Check-CsvHeaders ($CsvPath)
{
    $csv = Import-Csv $CsvPath
    $Headers = $csv|gm|where {$_.MemberType -eq 'NoteProperty'}|Select Name
    foreach ($h in $Headers){[string[]]$replaced += ($h -Replace ".*=") }
    foreach ($h in $replaced){[string[]]$csvHeaders += ($h -Replace "}") } # Creates a neat array of headers contained in the CSV file
    'Name','ScopeId','Router','Subnet','DnsDomain','DnsServer','StartRange','EndRange','SubnetMask','Description','ComputerName'|
    %{ if ($csvHeaders -notcontains $_){$i=1} }
    if ($i -gt 0){ return $false } else { return $true }
}
#endregion

#region DHCP Management Cmdlets
#=============================================================================# 
#                                                                             # 
...
function Parse-NetshClientLeaseResults( [array] $arrQueryBlob=$arrNetshClientLeases ) {
	...
	$arrQueryBlob | ForEach-Object {
		...
		if ( $_ -match $regLine ) {
			...
			$oClientLease.LeaseExpires = [datetime]"$($d[1])/$($d[0])/$($d[2])" #[datetime] $matches.LeaseExpires
